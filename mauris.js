...
playerStats.tutorialProgress++;
				...
				var asteroidDensity = parseInt(randomRange(5,25));
...
function headlightShadow(sprite,lightness){
				...
				for(var i=0;i<0.40;i+=0.10){
								...
								otherGraphics.lineTo(shadowSpot.x+Math.cos(rot + i)*2*Math.max(resolutionY,resolutionX),shadowSpot.y+Math.sin(rot + i)*2*Math.max(resolutionY,resolutionX));
								otherGraphics.lineTo(shadowSpot.x+(Math.sin(rot)*width),shadowSpot.y+(Math.cos(rot)*width));
								otherGraphics.lineTo(baseShadowSpot.x,baseShadowSpot.y);
								otherGraphics.lineTo(shadowSpot.x-(Math.sin(rot)*width),shadowSpot.y-(Math.cos(rot)*width));
								otherGraphics.endFill();
								shadowSpot.x+=Math.cos(rot)*width*0.2;
								shadowSpot.y+=Math.sin(rot)*width*0.2;
				}
				otherGraphics.blendMode=2;
...
function preload () {
				hello =  game.add.text(resolutionX*0.1,resolutionY*0.4, 'MAURIS\n\nLOADING',{ font:'36px acknowledge', fill: 'rgb(196,150,255)', align: 'left' })
...
playerShip.prototype.damage = function(dmg, aggro, bulletX, bulletY, preventXPLossOnDeath) {
				...
				if(this.shield){
								...
								if(this.health > 0){
												...
												game.add.tween(this.sprite).to({r:255,g:255,b:255,alpha:1},400, Phaser.Easing.Exponential.Out, true, 0, false);
												//handle part loss
												if(shipWithoutVoid(this.ship).length > 2 && Math.random() > 1.1 * Math.sqrt((((player.health * 1.33) / player.healthMax )- (Math.pow(dmg,0.33)/32)))){
																for(var i=0;i<this.parts.length;i++){
...
function portalGlow(explosionsGroup, x, y, btype, scaleFactor){
				...
				if(onscreen(x,y)){
								...
								if(forceDead(explosionsGroup)){
												...
												explosion.reset(x,y);
												explosion.rotation = Math.random()*Math.PI;
												explosion.body.velocity.x=randomRange(-10,10);
												explosion.body.velocity.y=randomRange(-10,10);
