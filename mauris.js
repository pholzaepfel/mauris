								...
								if(bullet){
												...
												bullet.bulletHitBehavior=[function(sprite,bullet){
																if(sprite.name=='player'&&playerStats.crew<player.crewMax){
																				playerStats.crew+=1;
																				ui.addDamageNumber(player.sprite.body.x,player.sprite.body.y,1,'+1 crew',true);
...
enemyShip.prototype.damage = function(dmg, aggro, bulletVelocity) {
				...
				if (this.health <= 0 && this.health + (damageCoef*dmg) >= 0){
								...
								if (this.ai==aiModes['asteroid'] && onscreen(this.sprite.x,this.sprite.y)){
												...
												if(Math.random()<0.08+(0.02*Math.sqrt(this.ship.length))){
...
function update () {
				...
				if(gamemode!='init'){
							...
							nebula2.reset(player.sprite.body.x*0.9980,player.sprite.body.y*0.9980);
								...
								planetfall.alpha=Math.max(0.3,planetfall.alpha);
